...
**Status:** **closed** em `9f1c14c` como `APPROVED_WITH_WARNINGS`. A auditoria original sobre
`307a3f0` permanece `REJECTED` no histórico; FIX-02, FIX-03 e FIX-04 fecharam D1–D4. Veredito
vigente em [`artifacts/acceptance-report.md`](artifacts/acceptance-report.md). **PB-05 está
liberado.**
...
| [PB-04-06](tasks/PB-04-06-construir-cenario-e-replay-da-hunt.md) | `buildHuntScenario`, fixture, golden e gate `hunt:check` | PB-04-04, PB-04-05 | Sol `xhigh` | done; replay fixed by FIX-01 |
...
| [PB-04-FIX-01](tasks/PB-04-FIX-01-corrigir-experiencia-da-hunt.md) | corrigir remix, composição, câmera, movimento, input e replay da hunt | PB-04-09 | Codex/GPT-5 | done; aceite de produto em `product-acceptance.md` |
| [PB-04-FIX-02](tasks/PB-04-FIX-02-tornar-o-toque-um-passo.md) | input edge-triggered; toque = um passo | PB-04-10 `REJECTED` D1 | Luna `xhigh` | done |
| [PB-04-FIX-03](tasks/PB-04-FIX-03-corrigir-hashes-congelados.md) | hashes de replay e `REPLAY_CONTRACT.md` | PB-04-10 `REJECTED` D2/D3 | Sol `xhigh` | done |
| [PB-04-FIX-04](tasks/PB-04-FIX-04-fechar-gates-e-contratos.md) | `biome check`, `retries: 0`, contratos | PB-04-10 `REJECTED` D4 | Luna `xhigh` | done |
| [PB-04-10](tasks/PB-04-10-auditar-e-fechar-playbook.md) | auditoria integrada e aceite | PB-04-FIX-02/03/04 + aceite pessoal | Opus 5 / Grok 4.6 | done — `APPROVED_WITH_WARNINGS` |
...
- [x] A seleção da hunt cumpre integralmente o checklist do roteiro e está congelada.
- [x] Toda criatura da tabela de spawn existe no catálogo PB-01.
- [x] `serverId == clientId` está provado para todos os IDs da região.
- [x] A tabela de flags é gerada do snapshot, versionada e reprodutível byte a byte.
- [x] Materializar o recipe versionado duas vezes produz JSON byte-idêntico. A geometria jogável é
...
- [x] Nenhuma transição necessária foi derrubada; a contagem bate com a seleção.
- [x] O kernel resolve andares, transições e spawn sem conhecer Tibia, asset ou path.
- [x] `events.golden.jsonl` do fixture PB-03 permaneceu byte-idêntico após o bump de schema.
- [x] Duas execuções limpas da sessão produzem snapshot e journal byte-idênticos.
- [x] Retomada converge em todas as fronteiras `0..600`, com `transitionGuard` e `spawnSlots`
      serializados.
- [x] Browser e Node produzem o mesmo SHA-256 do snapshot canônico da sessão.
- [x] Todo tile da região resolve por stable key; chave faltante falha em validação única.
- [x] A hunt é jogável nos quatro viewports obrigatórios, com screenshots versionadas.
- [x] `corepack pnpm verify` passa no resultado integrado.
- [x] O relatório de aceite decide a elegibilidade de PB-05.
...
Playbook fechado. A próxima task elegível vive em `docs/playbooks/PB-05/`. Abra um chat novo e envie
o bloco copiável de PB-05-01. Não antecipe nenhuma task além da indicada.
