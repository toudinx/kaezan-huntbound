...
describe('createCheckpointScheduler', () => {
  ...
  it('coalesces to one extra write of the newest state while a write is in flight', async () => {
    ...
    scheduler.onTick(4, () => makeSession({ huntId: 'hunt:tick-4' }));
    scheduler.onTick(8, () => makeSession({ huntId: 'hunt:tick-8' }));
    scheduler.onTick(12, () => makeSession({ huntId: 'hunt:tick-12' }));
    scheduler.onTick(16, () => makeSession({ huntId: 'hunt:tick-16' }));
  ...
  it('flush writes the pending checkpoint and resolves', async () => {
    ...
    scheduler.onTick(4, () => makeSession({ huntId: 'hunt:first' }));
    scheduler.onTick(8, () => makeSession({ huntId: 'hunt:pending' }));
  ...
  it('calls onError and does not throw from onTick when a write fails', async () => {
    ...
    expect(() => scheduler.onTick(4, () => makeSession())).not.toThrow();
