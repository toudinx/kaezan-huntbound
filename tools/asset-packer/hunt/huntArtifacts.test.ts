...
describe('generated hunt artifacts', () => {
  ...
  it('keeps source locks complete and the synthetic fixture deterministic', async () => {
    ...
    expect(sourceLock.files).toHaveLength(451);
  ...
  it('keeps the Hero Cave selection tied to lookType 73 and its loot', async () => {
    ...
    expect(selection.hunt?.keys).toHaveLength(270);
    expect(selection.entries).toHaveLength(270);
    expect(
  ...
  it('keeps the Hero Cave source lock complete and synthetic', async () => {
    ...
    expect(sourceLock.files).toHaveLength(270);
    expect(sourceLock.files.every((file) => file.byteLength === 68)).toBe(true);
  ...
  it('keeps the Dragon Lair selection tied to lookType 34 and its loot', async () => {
    ...
    expect(selection.hunt?.keys).toHaveLength(368);
    expect(selection.entries).toHaveLength(368);
    expect(
  ...
  it('keeps the Dragon Lair source lock complete and synthetic', async () => {
    ...
    expect(sourceLock.files).toHaveLength(368);
    expect(sourceLock.files.every((file) => file.byteLength === 68)).toBe(true);
