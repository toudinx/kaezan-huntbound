...
export const PUBLISHED_HASH_ARTIFACTS = [
  'scenario.json',
  'commands.jsonl',
...
export type PublishedHashArtifact = (typeof PUBLISHED_HASH_ARTIFACTS)[number];

const HASH_ROW = /^\| `([^`]+)` \| `([0-9a-f]{64})` \|$/;
const REQUIRED_ARTIFACTS = new Set<string>(PUBLISHED_HASH_ARTIFACTS);
...
export type CheckPublishedHashesResult =
  | {
      readonly ok: true;
      readonly digests: Readonly<Record<PublishedHashArtifact, string>>;
    }
  | {
      readonly ok: false;
...
export async function checkPublishedHashes(
  ...
  const errors: string[] = [];
  for (const artifact of PUBLISHED_HASH_ARTIFACTS) {
  ...
  const digests = {} as Record<PublishedHashArtifact, string>;
